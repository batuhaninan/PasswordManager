...
void mainpage::updateData(int row) {
  ...
  for (size_t i = 0; i < vectorSize; i++) {
    if (QString::fromStdString(data[i][0]) == selectedData[0] &&
        QString::fromStdString(data[i][2]) == selectedData[2]) {
...
void mainpage::updateDataInVector(int row,
    ...
    if (QString::fromStdString(data[i][0]) == selectedData[0] &&
      ...
      if (QString::fromStdString(data[i][1]) == selectedData[1]) {
        ...
        ui->tableWidget->setItem(
            row, 1,
            new QTableWidgetItem(QString::fromStdString(
                std::string(selectedData[1].size(), '*'))));
        ui->tableWidget->setCellWidget(row, 3, pWidget);
        break;
      } else {
        QWidget *pWidget = setIconForTable(hiddenEyeIconFilePath);
        ui->tableWidget->setItem(
            row, 1, new QTableWidgetItem(QString::fromStdString(data[i][1])));
        ui->tableWidget->setCellWidget(row, 3, pWidget);
        break;
      }
    }
  ...
  for (size_t i = 0; i < vectorSize; i++) {
    ...
    for (size_t j = 0; j < vectorSizeInside + 3; j++) {
      ...
      if (j == 0) {
        ...
        ui->tableWidget->setItem(order, USERNAME,
                                 new QTableWidgetItem(element));
      } else if (j == 1) {

        ui->tableWidget->setItem(order, PASSWORD,
                                 new QTableWidgetItem(QString::fromStdString(
                                     std::string(element.size(), '*'))));
      } else if (j == 2) {

        ui->tableWidget->setItem(order, DESCRIPTION,
                                 new QTableWidgetItem(element));
      } else if (j == 3) {
        QWidget *pWidget = setIconForTable(eyeIconFilePath);
        ui->tableWidget->setCellWidget(static_cast<int>(i), static_cast<int>(j),
                                       pWidget);
      } else if (j == 4) {
...
void mainpage::on_save_clicked() {
  // kullanıcının kaydetmek için girdiği username, password, description bilg
  // lerini kaydediyor ve tabloya ekliyor
  QString username, password, description;
